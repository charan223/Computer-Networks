...
printf("Enter file you want to download\n");
scanf("%s",file);
...
while(connect(sockfd,(struct sockaddr*)&peer_addr,sizeof(peer_addr))==-1)printf("error_connecting_to_peer\n");

while(send(sockfd,file,strlen(file),0)==-1)printf("send_error_to_peer\n");
...
if (recbytes==0){printf("connection_closed_by_peer\n");}

else{

//jk=0;

newfd=open(strcat(file,"(1)"),O_WRONLY|O_TRUNC|O_CREAT,S_IRWXU);
